...
if [ "$HYPRGAMEMODE" == "off" ] ; then
    ...
    echo "require(\"conf.animations.none\")" > ~/.config/hypr/conf/animations.lua
    ...
    echo "require(\"conf.look_feel.$CURRENT_THEME\")" > ~/.config/hypr/conf/look_feel.lua
    echo "require(\"conf.animations.$CURRENT_ANIMATIONS\")" > ~/.config/hypr/conf/animations.lua
